...
def rmse_analysis(p: int, mean_correlation: float, n_list: list, no_feat_cate:int, estimator = None, function = None) -> tuple[pd.DataFrame, dict]:
    dict_test = {}
    dict_train = {}
    ...
    for n in n_list:
        ...
        for i in range(5):
            ...
            if estimator=='OLS':
                estimated_cate_train, estimated_cate_test, RMSE_train, RMSE_test = method.ols_estimator(X_train, X_test, Y_train, T_train, T_test, true_cate_train, true_cate_test)
            elif estimator=='T-Learner':
                estimated_cate_train, estimated_cate_test, RMSE_test, RMSE_train = method.TLearner_estimator(Y_train, T_train, X_train, X_test, T_test, true_cate_train, true_cate_test)
            elif estimator=='CF DML':
                estimated_cate_train, estimated_cate_test, RMSE_test, RMSE_train = method.CF_DML(Y_train, T_train, X_train, X_test, T_test, true_cate_train, true_cate_test)
            elif estimator=='GRF':
                estimated_cate_train, estimated_cate_test, RMSE_test, RMSE_train = method.GRF_estimator(Y_train, T_train, X_train, X_test, T_test, true_cate_train, true_cate_test)
            else:
    ...
    return rmse_df
     ...
     #   return rmse_df
    #else:
     #   return rmse_df, coverage_dict





def get_rmse(p_list: list, n_list: list, no_feat_cate: int, mean_correlation: float, estimators: list, function=None) -> tuple[dict]:
    p_dict_rmse= {}
    #p_dict_coverage = {}
    ...
    for p in tqdm(p_list):
        ...
        for estimator in estimator_list:
          ...
          #      if estimator=='OLS' or estimator == 'T-Learner':
            ...
            #    else:
             #       rmse_simulation, coverage_dict = rmse_ci_analysis(p=p, mean_correlation=mean_correlation, n_list=n_list, no_feat_cate=no_feat_cate, estimator=estimator, function=function)
