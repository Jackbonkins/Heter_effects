...
def plot_cate_feat_val(p: int, n: int, mean_correlation: float, no_feat_cate: int, function: str, total=True):
    ...
    '''Produces a several visual plots for the data set features and their relationship with the CATE. 
       Features that determine the CATE functions are in red. If total is set to False, then only the relevant CATE features are plotted'''
...
def distr_cate(p:int, mean_correlation:float, n:int, function:str):

    '''Creates a histogram of CATE values. File name depends on the function input'''


    sim: SimulationStudy = SimulationStudy(p=p, mean_correlation=mean_correlation, cor_variance=0.2, n=n, no_feat_cate=3, non_linear=function)
    simulation = sim.create_dataset()

    cate = simulation['CATE']

    plt.figure(figsize=(8, 6))
    plt.hist(cate, bins=50, edgecolor='red', alpha=0.7, color='red')
    plt.xlabel('CATE')
    plt.ylabel('Frequency')

    plt.savefig(fname = f'CATE_{function}_distribution', bbox_inches = 'tight')